...
interface HostalBooking {
  id: string;
  hostal_id: string;
  hostal_nombre: string | null;
...
function MiniCalendar({ hostalId, bookings }: { hostalId: string; bookings: HostalBooking[] }) {
  ...
  const activeBookings = bookings.filter(
    (b) => b.hostal_id === hostalId && (b.estado === "confirmada" || b.estado === "pendiente")
  );
  ...
  return (
    <div className="bg-stone-50 rounded-xl p-4">
          ...
          return (
            <div
              ...
              className={`h-7 flex items-center justify-center rounded-md text-xs font-medium transition-colors
                ${booking
                  ? booking.estado === "confirmada" ? "bg-rose-500 text-white" : "bg-amber-400 text-white"
      ...
      <div className="flex items-center gap-3 mt-3 pt-3 border-t border-stone-200">
        <div className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-sm bg-rose-500"></div><span className="text-xs text-stone-500">Confirmada</span></div>
...
export default function HostalBookingsSection() {
  const [bookings, setBookings] = useState<HostalBooking[]>([]);
  const [loading, setLoading] = useState(true);
  const [selected, setSelected] = useState<HostalBooking | null>(null);
  const [filterEstado, setFilterEstado] = useState<"todos" | EstadoReserva>("todos");
  const [filterHostal, setFilterHostal] = useState("todos");
  const [search, setSearch] = useState("");
  const [saving, setSaving] = useState(false);
  const [successMsg, setSuccessMsg] = useState<string | null>(null);
  const [showCalendar, setShowCalendar] = useState(false);
  const [calendarHostalId, setCalendarHostalId] = useState<string | null>(null);
  const [metodoPago, setMetodoPago] = useState<"Revolut" | "Bizum" | "Efectivo" | "Transferencia">("Revolut");
  const [showNewModal, setShowNewModal] = useState(false);
  const [newForm, setNewForm] = useState({
    hostal_nombre: "", cliente_nombre: "", cliente_email: "", cliente_telefono: "",
    fecha_inicio: "", fecha_fin: "", tipo_habitacion: "Habitación doble",
  ...
  const fetchBookings = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from("hostal_bookings").select("*").order("fecha_inicio", { ascending: false });
    if (data) setBookings(data as HostalBooking[]);
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    const channel = supabase
      .channel("hostal_bookings_admin")
      .on("postgres_changes", { event: "*", schema: "public", table: "hostal_bookings" }, () => { fetchBookings(); })
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [fetchBookings]);

  const hostales = Array.from(new Set(bookings.map((b) => b.hostal_id))).map((id) => ({
    id,
    nombre: bookings.find((b) => b.hostal_id === id)?.hostal_nombre || id,
  }));

  const filtered = bookings.filter((b) => {
    const matchEstado = filterEstado === "todos" || b.estado === filterEstado;
    const matchHostal = filterHostal === "todos" || b.hostal_id === filterHostal;
    const q = search.toLowerCase();
    const matchSearch = !q || (b.cliente_nombre || "").toLowerCase().includes(q) || (b.hostal_nombre || "").toLowerCase().includes(q);
    return matchEstado && matchHostal && matchSearch;
  });
  ...
  const updateEstado = async (id: string, estado: EstadoReserva) => {
    setSaving(true);
    const { error } = await supabase.from("hostal_bookings").update({ estado, updated_at: new Date().toISOString() }).eq("id", id);
    if (!error) {
          ...
          try {
            await supabase.functions.invoke("whatsapp-notify", {
              body: {
                ...
                propiedad: `Hostal: ${booking.hostal_nombre}`,
  ...
  const markAsPaid = async (id: string) => {
    setSaving(true);
    const { error } = await supabase.from("hostal_bookings").update({ metodo_pago: metodoPago, updated_at: new Date().toISOString() }).eq("id", id);
    if (!error) {
  ...
  const handleCreateBooking = async () => {
    if (!newForm.hostal_nombre || !newForm.cliente_nombre || !newForm.fecha_inicio || !newForm.fecha_fin) return;
    setNewSaving(true);
    ...
    const { data, error } = await supabase.from("hostal_bookings").insert({
      hostal_id: newForm.hostal_nombre.toLowerCase().replace(/\s+/g, "-"),
      hostal_nombre: newForm.hostal_nombre,
      ...
      tipo_habitacion: newForm.tipo_habitacion,
    ...
    if (!error && data) {
      setBookings((prev) => [data as HostalBooking, ...prev]);
      showSuccess("Reserva creada correctamente");
      setShowNewModal(false);
      setNewForm({ hostal_nombre: "", cliente_nombre: "", cliente_email: "", cliente_telefono: "", fecha_inicio: "", fecha_fin: "", tipo_habitacion: "Habitación doble", num_personas: 2, precio_base: 0, notas: "" });
    }
    setNewSaving(false);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-semibold text-stone-900 flex items-center gap-2">
            <i className="ri-building-2-line text-rose-600"></i>
            Reservas de Hostales
          </h2>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 rounded-full text-sm font-medium border transition-colors cursor-pointer whitespace-nowrap ${showCalendar ? "bg-rose-600 text-white border-transparent" : "bg-white border-stone-200 text-stone-600 hover:border-stone-400"}`}
          >
        ...
        <div className="bg-white rounded-2xl border border-stone-100 p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-stone-900 text-sm flex items-center gap-2">
              <i className="ri-calendar-2-line text-rose-600"></i>
              Disponibilidad por hostal
            </h3>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {hostales.map((h) => (
              <button
                key={h.id}
                onClick={() => setCalendarHostalId(calendarHostalId === h.id ? null : h.id)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all cursor-pointer whitespace-nowrap ${calendarHostalId === h.id ? "bg-rose-600 text-white border-transparent" : "bg-white border-stone-200 text-stone-600 hover:border-rose-300"}`}
              >
                <i className="ri-building-2-line mr-1"></i>{h.nombre}
              </button>
            ))}
            {hostales.length === 0 && <p className="text-xs text-stone-400">No hay hostales con reservas aún</p>}
          </div>
          {calendarHostalId && (
            <div className="max-w-sm">
              <p className="text-xs font-semibold text-stone-600 mb-3">
                <i className="ri-building-2-line mr-1 text-rose-600"></i>
                {hostales.find((h) => h.id === calendarHostalId)?.nombre}
              </p>
              <MiniCalendar hostalId={calendarHostalId} bookings={bookings} />
              <div className="mt-4 space-y-2">
                <p className="text-xs font-semibold text-stone-500 uppercase tracking-wide">Reservas activas</p>
                {bookings
                  .filter((b) => b.hostal_id === calendarHostalId && (b.estado === "confirmada" || b.estado === "pendiente"))
                  .map((b) => (
                    <div key={b.id} onClick={() => { setSelected(b); setShowCalendar(false); }} className="flex items-center justify-between bg-stone-50 rounded-lg px-3 py-2.5 cursor-pointer hover:bg-stone-100 transition-colors">
                      <div>
                        ...
                        <p className="text-xs text-stone-500">{formatDate(b.fecha_inicio)} → {formatDate(b.fecha_fin)} · {b.tipo_habitacion}</p>
                      </div>
                ...
                {bookings.filter((b) => b.hostal_id === calendarHostalId && (b.estado === "confirmada" || b.estado === "pendiente")).length === 0 && (
                  <p className="text-xs text-stone-400 text-center py-3">Sin reservas activas</p>
                )}
              </div>
            </div>
          )}
          {!calendarHostalId && hostales.length > 0 && <p className="text-xs text-stone-400 text-center py-6">Selecciona un hostal para ver su calendario</p>}
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-xs">
          ...
          <input type="text" placeholder="Buscar cliente u hostal..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2.5 border border-stone-200 rounded-full text-sm focus:outline-none focus:border-stone-400 bg-white" />
        </div>
        <select value={filterHostal} onChange={(e) => setFilterHostal(e.target.value)} className="border border-stone-200 rounded-full px-4 py-2.5 text-sm focus:outline-none focus:border-stone-400 bg-white cursor-pointer text-stone-600">
          <option value="todos">Todos los hostales</option>
          {hostales.map((h) => <option key={h.id} value={h.id}>{h.nombre}</option>)}
        </select>
        ...
        <div className="flex-1 space-y-3">
          {loading ? (
            ...
            <div className="text-center py-16 bg-white rounded-xl border border-stone-100">
              <div className="w-14 h-14 flex items-center justify-center rounded-full bg-rose-50 text-rose-400 mx-auto mb-3">
              ...
              <div key={b.id} onClick={() => setSelected(b)} className={`bg-white rounded-xl border p-4 cursor-pointer transition-all hover:border-stone-300 ${selected?.id === b.id ? "border-stone-400 ring-1 ring-stone-200" : "border-stone-100"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-start gap-3 min-w-0">
                    <div className="w-10 h-10 flex items-center justify-center rounded-xl bg-rose-50 text-rose-600 flex-shrink-0">
                      <i className="ri-building-2-line text-base"></i>
                    </div>
                    <div className="min-w-0">
                      <div className="text-stone-900 font-semibold text-sm truncate">{b.hostal_nombre || "Hostal"}</div>
                      <div className="text-stone-500 text-xs truncate mt-0.5">
                        <i className="ri-user-line mr-1"></i>{b.cliente_nombre || "Cliente"}
                        {b.tipo_habitacion && <span className="ml-2 text-stone-400">· {b.tipo_habitacion}</span>}
                      </div>
          ...
          <div className="w-full lg:w-96 bg-white rounded-2xl border border-stone-100 p-6 h-fit sticky top-6 space-y-4">
            ...
            <div className="bg-rose-50 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-1">
                <i className="ri-building-2-line text-rose-600 text-sm"></i>
                <span className="text-xs font-semibold text-rose-700 uppercase tracking-wide">Hostal</span>
              </div>
              <p className="text-stone-900 font-semibold text-sm">{selected.hostal_nombre || "—"}</p>
              {selected.tipo_habitacion && (
                <span className="text-xs bg-rose-100 text-rose-700 px-2 py-0.5 rounded-full mt-1 inline-block">{selected.tipo_habitacion}</span>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-red-50 rounded-xl p-3 text-center">
                <p className="text-xs text-red-500 font-medium mb-1">Check-out</p>
                ...
                <div className="mt-3 pt-3 border-t border-stone-700 space-y-1.5">
                  {selected.precio_base && selected.precio_base > 0 && (
                    <div className="flex justify-between text-xs text-stone-400">
                      <span>Base alojamiento</span><span>€{selected.precio_base.toLocaleString("es-ES")}</span>
                    </div>
                  )}
                  {selected.comision_nexura && selected.comision_nexura > 0 && (
                    <div className="flex justify-between text-xs text-stone-400">
                      <span>Comisión NEXURA (15%)</span><span>€{selected.comision_nexura.toLocaleString("es-ES")}</span>
                    </div>
                  )}
                  {selected.iva_comision && selected.iva_comision > 0 && (
                    <div className="flex justify-between text-xs text-amber-400 font-semibold">
                      <span>IVA 21% (mediación)</span><span>€{selected.iva_comision.toLocaleString("es-ES")}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-2 pt-2 border-t border-stone-100">
              ...
              {selected.estado === "pendiente" && (
                <button onClick={() => updateEstado(selected.id, "confirmada")} disabled={saving} className="w-full bg-emerald-600 text-white py-3 rounded-full text-xs font-semibold hover:bg-emerald-500 cursor-pointer whitespace-nowrap transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                  <i className="ri-check-double-line"></i>Confirmar reserva — bloquear fechas
                </button>
              )}
              ...
              {(selected.estado === "pendiente" || selected.estado === "confirmada") && (
                <button onClick={() => updateEstado(selected.id, "cancelada")} disabled={saving} className="w-full border border-red-200 text-red-500 py-3 rounded-full text-xs font-medium hover:bg-red-50 cursor-pointer whitespace-nowrap transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                  <i className="ri-close-circle-line"></i>Cancelar — liberar fechas en calendario
                </button>
              )}
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-semibold text-stone-900 flex items-center gap-2">
                <i className="ri-building-2-line text-rose-600"></i>Nueva reserva de hostal
              </h3>
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-medium text-stone-600 mb-1.5 block">Hostal</label>
                <input type="text" placeholder="Nombre del hostal" value={newForm.hostal_nombre} onChange={(e) => setNewForm((f) => ({ ...f, hostal_nombre: e.target.value }))} className="w-full border border-stone-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-stone-400 bg-stone-50" />
              </div>
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="text-xs font-medium text-stone-600 mb-1.5 block">Check-in</label>
                ...
                <div>
                  <label className="text-xs font-medium text-stone-600 mb-1.5 block">Check-out</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs font-medium text-stone-600 mb-1.5 block">Tipo de habitación</label>
                  <select value={newForm.tipo_habitacion} onChange={(e) => setNewForm((f) => ({ ...f, tipo_habitacion: e.target.value }))} className="w-full border border-stone-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-stone-400 bg-stone-50 cursor-pointer">
              ...
              {newForm.precio_base > 0 && newForm.fecha_inicio && newForm.fecha_fin && new Date(newForm.fecha_fin) > new Date(newForm.fecha_inicio) && (() => {
                ...
                return (
                  <div className="bg-stone-50 rounded-xl border border-stone-200 p-4 space-y-2">
                    ...
                    <div className="flex justify-between text-xs text-stone-600"><span>Alojamiento ({noches} noches × €{newForm.precio_base})</span><span>€{subtotal.toLocaleString("es-ES")}</span></div>
              ...
              <div>
                ...
                <textarea rows={2} maxLength={500} placeholder="Peticiones especiales, alergias, etc..." value={newForm.notas} onChange={(e) => setNewForm((f) => ({ ...f, notas: e.target.value }))} className="w-full border border-stone-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-stone-400 bg-stone-50 resize-none" />
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleCreateBooking} disabled={newSaving || !newForm.hostal_nombre || !newForm.cliente_nombre || !newForm.fecha_inicio || !newForm.fecha_fin} className="flex-1 bg-stone-900 text-white py-3 rounded-full text-sm font-semibold hover:bg-stone-700 cursor-pointer whitespace-nowrap transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                {newSaving ? <><i className="ri-loader-4-line animate-spin mr-1"></i>Creando...</> : <><i className="ri-building-2-line mr-1"></i>Crear reserva</>}
              </button>
            </div>
          </div>
        </div>
