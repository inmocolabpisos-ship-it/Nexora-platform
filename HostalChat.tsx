...
const QUICK_QUESTIONS = [
  "¿Hay disponibilidad para este fin de semana?",
  "¿Incluye desayuno?",
  "¿Tienen parking?",
  "¿Admiten mascotas?",
];

export default function HostalChat({ nombreHostal }: { nombreHostal: string }) {
  const [open, setOpen] = useState(false);
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      text: `¡Hola! Soy el equipo de ${nombreHostal}. ¿En qué podemos ayudarte?`,
      sender: "host",
  ...
  const sendMessage = async (text: string) => {
    ...
    try {
      await supabase.functions.invoke("whatsapp-notify", {
        body: {
          tipo: "chat_hostal",
          pagina: `Hostal: ${nombreHostal}`,
          propiedad: nombreHostal,
          mensaje: text.trim(),
        },
      });
    } catch {
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-40 w-14 h-14 flex items-center justify-center rounded-full bg-stone-900 text-white cursor-pointer hover:bg-stone-700 transition-colors"
        aria-label="Chat con el hostal"
      >
      ...
      {open && (
        <div className="fixed bottom-24 right-6 z-50 w-80 bg-white rounded-2xl border border-stone-200 flex flex-col overflow-hidden" style={{ maxHeight: "480px" }}>
          ...
          <div className="flex items-center gap-3 px-4 py-3 bg-stone-900 text-white">
            <div className="w-9 h-9 flex items-center justify-center rounded-full bg-amber-500 flex-shrink-0">
              <i className="ri-hotel-line text-sm"></i>
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold truncate font-body">{nombreHostal}</p>
            ...
            <div className="px-3 py-2 bg-white border-t border-stone-100 flex flex-wrap gap-1.5">
              {QUICK_QUESTIONS.map((q) => (
                <button
                  ...
                  className="text-xs bg-stone-100 text-stone-600 px-2.5 py-1.5 rounded-full hover:bg-amber-100 hover:text-amber-700 transition-colors cursor-pointer font-body whitespace-nowrap"
                >
                  {q}
                </button>
              ))}
          ...
          <div className="flex items-center gap-2 px-3 py-3 bg-white border-t border-stone-100">
            ...
            <button
              ...
              className="w-8 h-8 flex items-center justify-center rounded-full bg-stone-900 text-white cursor-pointer hover:bg-stone-700 transition-colors disabled:opacity-40"
            >
